...
HANDLERS.cminfo = async function ({ sock, jid, args, sender, isGroup }) {
  ...
  return [chosen[0]];
    ...
    function formatPlayerHistory(playerTag, opts = { limit: 10, consider: 500 }) {
      ...
      // work with newest-first
const logsNewestFirst = playerLogsRaw
  .filter(l => !l.isFromLive || l.finalized)   // 👈 live snapshots skip karo
  .slice(-opts.consider)
  .reverse();
...
// ------------------------- AUTO-SAVE CURRENT NORMAL WAR ATTACKS (robust) -------------------------
setInterval(async () => {
  ...
  try {
    ...
    for (const jid in DB.userClans) {
      ...
      const warData = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar`);
      // Skip if no normal war running (or it's CWL)
      ...
      // canonical keys
      const warKeyBase = `${warData.clan.tag}_${warData.opponent.tag}`; // short id for match-up
      const warIdFull = `${warKeyBase}_${warData.endTime}`; // unique for this war (includes endTime)

      for (const m of warData.clan.members) {
        const playerTag = m.tag;
        DB.playerWarLogs[playerTag] = DB.playerWarLogs[playerTag] || [];

        const attacks = m.attacks || [];

        // keep logs that are NOT for this warKeyBase (so we can replace this war's snapshots)
        const existingOtherLogs = DB.playerWarLogs[playerTag].filter(l => l.warIdShort !== warKeyBase);
        const newLogs = [];

        // Save only real non-empty attacks (avoid saving zero/duplicate empty snapshots)
        for (const atk of attacks) {
          const order = Number(atk.order ?? atk.attackNumber ?? 0);
          const warKey = `${warKeyBase}_${playerTag}_${order}`;

          // already saved for this war+order?
          if (existingOtherLogs.concat(newLogs).some(l => l.warKey === warKey)) continue;

          // skip empty snapshots (0 stars && 0%)
          const stars = Number(atk.stars || 0);
          const pct = Number(atk.destructionPercentage || 0);
          if (stars === 0 && pct === 0) continue;
          ...
          newLogs.push({
            ...
            order,
            defenderTag: atk.defenderTag || null,
            isFromLive: true,
            finalized: false,
            unused: false,
        ...
        // If no actual attacks saved and there is NO existing placeholder for this war, create a single placeholder:
        const hasPlaceholderAlready = existingOtherLogs.some(l => l.warIdShort === warKeyBase && l.unused === true);
        if (newLogs.length === 0 && attacks.length === 0 && !hasPlaceholderAlready) {
          newLogs.push({
            warKey: `${warIdFull}_${playerTag}_unused`,
            warId: warIdFull,
            warIdShort: warKeyBase,
            clanTag: warData.clan.tag,
            oppClanTag: warData.opponent.tag,
        ...
        // write back compacted logs (other wars kept, this war replaced by newLogs)
        DB.playerWarLogs[playerTag] = existingOtherLogs.concat(newLogs);
    ...
    console.error("Auto warlog save error:", e && e.message ? e.message : e);
  }
}, 5 * 60 * 1000);


// --------------------------- CWL SNAPSHOT (INWAR ONLY) (robust) ---------------------------
async function saveLiveCWLWarSnapshots() {
  ...
  try {
    ...
    for (const userKey in DB.userClans) {
      ...
      // Fetch league group
      const leagueGroup = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar/leaguegroup`);
      if (!leagueGroup || !Array.isArray(leagueGroup.rounds)) continue;

      let cwlWar = null, myClan = null, warIdFull = '', roundIndex = -1, warKeyBase = '';
      for (let i = leagueGroup.rounds.length - 1; i >= 0; i--) {
        const round = leagueGroup.rounds[i];
        if (!Array.isArray(round.warTags)) continue;
        for (const warTag of round.warTags) {
          ...
          if (war.state === 'inWar' &&
            ...
            cwlWar = war;
            warKeyBase = `${war.clan.tag}_${war.opponent.tag}`; // short base
            warIdFull = `${warKeyBase}_${war.endTime}`;
            roundIndex = i + 1;
            break;
          }
        }
      ...
      for (const m of myClan.members) {
        ...
        // keep logs that are NOT for this exact CWL round (remove previous snapshots for same round)
        const oldLogs = DB.playerWarLogs[playerTag].filter(l => !(l.warId === warIdFull || l.cwlRound === roundIndex));
        const newLogs = [];

        // Save each actual attack (may be multiple per player in CWL depending on format)
        for (const atk of attacks) {
          const order = Number(atk.order ?? atk.attackNumber ?? 0);
          const warKey = `${warIdFull}_${playerTag}_${order}`;
          ...
          if (atk.defenderTag) {
            try {
              ...
              // ignore defender fetch error, use Unknown
            }
          }

          // skip empty attack snapshots (0% && 0⭐)
          const stars = Number(atk.stars || 0);
          const pct = Number(atk.destructionPercentage || 0);
          if (stars === 0 && pct === 0) continue;
        ...
        // If no actual attacks, save ONE placeholder for this CWL round (if not already present)
        const hadPlaceholder = oldLogs.some(l => l.warId === warIdFull && l.unused === true && l.cwlRound === roundIndex);
        if (newLogs.length === 0 && attacks.length === 0 && !hadPlaceholder) {
          newLogs.push({
            warKey: `${warIdFull}_${playerTag}_unused`,
            warId: warIdFull,
            warIdShort: warKeyBase,
    ...
    console.error("❌ Error in CWL snapshot:", e && e.message ? e.message : e);
  }
}
...
let isGetLogWarRunning = false;
let isCwlGetWarRunning = false;

// Helper: Spawn a script and wait for finish
...
// Function: getlogwar.js (3 min interval)
async function runGetLogWar() {
  if (isGetLogWarRunning) {
    console.log("runGetLogWar already running. Skipping...");
    return;
  }
  isGetLogWarRunning = true;
    ...
    for (const clanTag of clans) {
      try {
        ...
        if (!warData || warData.state === "inWar") {
          console.log(`Skipping getlogwar for ${clanTag}, state=${warData?.state}`);
          continue;
        }

        await runScript("getlogwar.js");
        console.log(`getlogwar.js completed for clan ${clanTag}`);
      } catch (e) {
        console.error(`Error in getlogwar for clan ${clanTag}: ${e.message}`);
      }
    }
  } catch (err) {
    console.error("runGetLogWar fatal error:", err.message);
  } finally {
    isGetLogWarRunning = false;
  }
}

// Function: cwlgetwar.js (8 min interval)
async function runCwlGetWar() {
  if (isCwlGetWarRunning) {
    console.log("runCwlGetWar already running. Skipping...");
    return;
  }
  isCwlGetWarRunning = true;
  ...
  try {
    ...
    for (const clanTag of clans) {
      try {
        const cwlWarData = await cocFetch(`/clans/${encodeURIComponent(clanTag)}/currentwar`);
        if (!cwlWarData || cwlWarData.state !== "warEnded") {
          console.log(`Skipping CWL finalize for ${clanTag}, state=${cwlWarData?.state}`);
          continue;
        }

        await runScript("cwlgetwar.js");
        console.log(`cwlgetwar.js completed for clan ${clanTag}`);
      } catch (e) {
        console.error(`Error in cwlgetwar for clan ${clanTag}: ${e.message}`);
      }
    }
  } catch (err) {
    console.error("runCwlGetWar fatal error:", err.message);
  } finally {
    isCwlGetWarRunning = false;
  }
}

// Run immediately & set intervals
runGetLogWar();
setInterval(runGetLogWar, 3 * 60 * 1000);

runCwlGetWar();
setInterval(runCwlGetWar, 8 * 60 * 1000);
